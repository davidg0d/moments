...
export default function StoreFront() {
  ...
  const handleNonAuthCheckout = async (data: CheckoutFormValues) => {
    ...
    try {
      ...
      // Enviar pedido para o servidor usando a rota para visitantes
      const response = await apiRequest(
        "POST", 
        `/api/non-auth-orders/${store.id}`, 
        orderData
      );
      ...
      const message = `🛍️ *NOVO PEDIDO - ${store.name}* 🛍️\n\n` +
      ...
      // Redirecionar para o WhatsApp (usar URL da resposta da API se disponível)
      const whatsappUrl = responseData.whatsappLink || 
      ...
      toast({
        ...
        description: `Pedido #${responseData.order.id} foi criado.`,
      });
  ...
  return (
    <div className="min-h-screen" style={{
      ...
      <header>
        <div className="container mx-auto px-4 py-10 bg-[#00012E] shadow-md text-white">
          <div className="flex flex-col md:flex-row items-center justify-between md:justify-between gap-6 w-full">
            {/* Logo da Empresa na versão mobile - acima do logo de consultor */}
            <div className="flex w-full md:hidden justify-center items-center mb-2">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
              {store.logoUrl && (
                <div className="h-32 w-32 md:h-40 md:w-40 overflow-hidden rounded-full bg-gray-100 border-4 border-white shadow-xl">
                  <img
                    src={store.logoUrl}
                    alt={store.name}
              ...
              <div className="text-center md:text-left">
                <h1 className="text-2xl md:text-4xl font-bold text-white">{store.name}</h1>
                <p className="text-sm md:text-lg text-white mt-2 mb-3 max-w-xs md:max-w-md">
                  {store.description?.split(/\s+/).map((word, i, words) => (
                    <span key={i} className="whitespace-pre-wrap">
                      {word}
                      {/* Adicionar quebra de linha a cada 4 palavras - menos intenso */}
                      {(i + 1) % 4 === 0 && i !== words.length - 1 && <br />}
                      {(i + 1) % 4 !== 0 && i !== words.length - 1 && " "}
                    </span>
                  ...
                  <div>
                    {store.whatsappNumber && (
                      <a
                        ...
                        className="inline-flex items-center justify-center w-12 h-12 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-full transition-colors"
            ...
            {/* Logo da Empresa (versão desktop) - posicionado quase no meio */}
            <div className="hidden md:flex w-48 h-32 absolute left-1/2 transform -translate-x-1/2 items-center">
              {store.companyLogoUrl ? (
                <img
                  src={store.companyLogoUrl}
                  alt={`Logo da empresa ${store.name}`}
      ...
      <div className="fixed bottom-4 right-4 z-40">
        <Sheet>
          <SheetTrigger asChild>
            <Button className="relative bg-[#00012E] hover:bg-[#00023F] text-white rounded-full h-14 w-14 flex items-center justify-center">
              <ShoppingBag className="h-6 w-6" />
              {cartItems.length > 0 && (
                <span className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-[#25D366] text-xs text-white">
                  {cartItems.length}
                </span>
              )}
            </Button>
            ...
            {cartItems.length > 0 ? (
              <>
                ...
                <div className="space-y-4">
                  ...
                  <Button
                    className="w-full bg-[#00012E] hover:bg-[#00023F] text-white"
              ...
              .map((product) => (
                <Card key={product.id} className="overflow-hidden">
                  ...
                  <CardHeader className="p-3 md:pb-2 md:p-6">
                    <CardTitle 
                      className="text-sm md:text-lg cursor-pointer hover:text-[#00012E]" 
                      onClick={() => openProductDetails(product)}
                    >
                      {product.name}
                    </CardTitle>
                  ...
                  <CardFooter className="p-3 md:p-6">
                    <Button
                      className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white text-xs md:text-sm"
                      ...
                      <FormControl>
                        ...
                        >
                          ...
                          <FormItem className="flex items-center space-x-3 space-y-0">
                            ...
                            <FormLabel className="font-normal">
                              Entrega (informe o endereço abaixo)
                            </FormLabel>
                          </FormItem>
                        </RadioGroup>
                      </FormControl>
          ...
          {selectedProduct && (
            <>
              <DialogHeader>
                ...
                <DialogDescription className="text-lg font-medium text-[#00012E]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
                ...
                {/* Detalhes do produto */}
                <div className="space-y-4">
                  <h3 className="text-lg font-medium">Descrição do Produto</h3>
